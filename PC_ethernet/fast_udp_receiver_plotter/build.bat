...
cl /nologo /O2 /EHsc /std:c++17 ^
    ...
    src\main.cpp ^
    src\ConsoleUI.cpp ^
    src\NpcapReceiver.cpp ^
    src\PacketParser.cpp ^
    src\SampleDecoder.cpp ^
    src\Utils.cpp ^
    /Fe:build\fast_udp_receiver.exe ^
    /link /LIBPATH:"%NPCAP_SDK%\Lib\x64" wpcap.lib Packet.lib Ws2_32.lib
if errorlevel 1 ( echo Build failed. & exit /b 1 )
echo.
echo Build OK: build\fast_udp_receiver.exe
echo Run as Administrator: build\fast_udp_receiver.exe
echo Plot in another terminal: python scripts\plot_live.py output\samples.csv --channels 0
